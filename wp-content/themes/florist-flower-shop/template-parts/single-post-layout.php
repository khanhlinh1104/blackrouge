...
<article id="post-<?php the_ID(); ?>" <?php post_class('inner-service'); ?>>
    <h1><?php the_title(); ?></h1>
    <?php if( get_theme_mod( 'florist_flower_shop_toggle_postdate',true) != '' || get_theme_mod( 'florist_flower_shop_toggle_author',true) != '' || get_theme_mod( 'florist_flower_shop_toggle_comments',true) != '' || get_theme_mod( 'florist_flower_shop_toggle_time',true) != '') { ?>
        <div class="post-info p-2 my-3">
          ...
          <?php if(get_theme_mod('florist_flower_shop_toggle_author',true)==1){ ?>
            <span><?php echo esc_html(get_theme_mod('florist_flower_shop_single_post_meta_field_separator', '|'));?></span> <i class="fas fa-user"></i><span class="entry-author"><a href="<?php echo esc_url( get_author_posts_url( get_the_author_meta( 'ID' )) ); ?>"><?php the_author(); ?><span class="screen-reader-text"><?php the_author(); ?></span></a></span>
          <?php } ?>

          <?php if(get_theme_mod('florist_flower_shop_toggle_comments',true)==1){ ?>
            <span><?php echo esc_html(get_theme_mod('florist_flower_shop_single_post_meta_field_separator', '|'));?></span> <i class="fa fa-comments" aria-hidden="true"></i><span class="entry-comments"><?php comments_number( __('0 Comment', 'florist-flower-shop'), __('0 Comments', 'florist-flower-shop'), __('% Comments', 'florist-flower-shop') ); ?></span>
          <?php } ?>

            <?php if(get_theme_mod('florist_flower_shop_toggle_time',true)==1){ ?>
              <span><?php echo esc_html(get_theme_mod('florist_flower_shop_single_post_meta_field_separator', '|'));?></span> <i class="fas fa-clock"></i><span class="entry-time"><?php echo esc_html( get_the_time() ); ?></span>
            <?php } ?>
        </div>
    <?php } ?>
