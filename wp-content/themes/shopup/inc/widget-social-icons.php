 ...
 * Register widget
 */
function shopup_register_widget() {
	register_widget( 'ShopUp_Social_Icons_Widget' );
}
add_action( 'widgets_init', 'shopup_register_widget' );
 ...
 * @since 1.0.0
 */
class ShopUp_Social_Icons_Widget extends WP_Widget {
	...
	public function __construct() {
		parent::__construct(
			...
			'shopup_social_icons',
			// widget name.
			__( 'ShopUp Social Icons', 'shopup' ),
			// widget description.
			array( 'description' => __( 'Adds a social icon menu.', 'shopup' ) )
		);
	}
	...
	public function form( $instance ) {
		...
		if ( ! $menus ) {
			...
			echo '<p>' . sprintf( __( 'No menus have been created yet. <a href="%s">Create some</a>.', 'shopup' ), esc_url( admin_url( 'nav-menus.php' ) ) ) . '</p>'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
			return;
		}
		?>
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'title' ) ); ?>"><?php esc_html_e( 'Title', 'shopup' ); ?></label>
			<input type="text" class="widefat" id="<?php echo esc_attr( $this->get_field_id( 'title' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'title' ) ); ?>" value="<?php echo esc_attr( $title ); ?>" />
		</p>
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'nav_menu' ) ); ?>"><?php esc_html_e( 'Select Menu:', 'shopup' ); ?></label>
			<select id="<?php echo esc_attr( $this->get_field_id( 'nav_menu' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'nav_menu' ) ); ?>">
				<option value="0"><?php esc_html_e( '&mdash; Select &mdash;', 'shopup' ); ?></option>
				<?php
